...
const getAllData = async (req,res) => {
    try {
        ...
        const data = await Album.find().populate('cat_id').limit(limit).skip(skip);
        const totalData = await Album.countDocuments();
...
const getDataByID = async (req,res) => {
    try {
        const data = await Album.findOne( {_id: req.params.id} ).populate('cat_id');
